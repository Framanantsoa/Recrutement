...
public class RequestValidationRepository : IRequestValidationRepository
{
    ...
    public async Task<RequestStatus> GetNextStatus(RecruitmentRequest req) {
        var nextValidator = await GetNextValidator(req.Id);

        string statusId = nextValidator == null ? "STD_003" : "STD_002"; // Validée ou En cours

        return await _dbCtx.RequestStatuses
            .AsNoTracking()
            .FirstAsync(s => s.Id == statusId);
    }
    ...
    public async Task<(RecruitmentRequest, bool)> ValidateRequest(CreateRequestValidationDTO data) {
        ...
        var validators = await this.GetAllDirectorValidator(request.Id);
        if (!validators.Any(v => v.UserId == userValidator.UserId))
            throw new ArgumentException("Utilisateur non accordé pour valider");

        // Vérification que l'utilisateur n'a pas déjà validé
        if (!await this.HasNotYetValidatedRequest(userValidator, request))
            throw new ArgumentException("Demande déjà validée par le même utilisateur");

        // 🔥 1. Marquer le validateur comme validé dans RequestsPerValidators
        var reqValidator = await _dbCtx.RequestsPerValidators
            .FirstOrDefaultAsync(r => r.RequestId == request.Id && r.ValidatorId == userValidator.UserId);

        if (reqValidator != null)
            reqValidator.IsValidated = true;

        // 🔥 Sauvegarder maintenant pour que GetNextStatus voie le dernier validateur
        await _dbCtx.SaveChangesAsync();

        // 2. Créer l'objet RequestValidation
