    ...
    ;WITH LastVal AS (
        SELECT 
            ...
            ROW_NUMBER() OVER (
                ...
                ORDER BY rv.created_at DESC, rv.request_validation_id DESC
            ) AS rn
...
-- 3. Procédure pour le Directeur Général
...
-- 4. Procédure pour les directeurs de département
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT 
            ...
            u.position
        FROM users u
        LEFT JOIN users sup ON u.superior_id = sup.user_id
        WHERE 
            ...
            -- uniquement DIRECTEUR / DIRECTRICE  en début
            AND (
                UPPER(u.position) LIKE 'DIRECTEUR %'
                OR UPPER(u.position) LIKE 'DIRECTRICE %'
                OR UPPER(u.position) LIKE 'DIRECTOR %'
            )
        ...
        WHEN MATCHED THEN
            UPDATE SET
                ...
                target.superior_id = source.superior_id,
        ...
        -- supprimer les anciens directeurs obsolètes
        WHEN NOT MATCHED BY SOURCE
            AND target.validator_type = 'Directeur de tutelle'
        THEN DELETE;
...
-- 6. Procédure pour les chefs de département
...
-- 7. Procédure principale pour mettre à jour tous les validateurs
...
-- 8. Procédure pour réinitialiser validators_flow
