...
public class RecruitmentRequestRepository : IRecruitmentRequestRepository
{
    ...
    public async Task<string> GenerateRequestId(string direction, int year) {
        var key = $"REQ_{direction}_{year}";
        var number = await _generator.GetNextValueAsync(key);

    // Ex : DR-DRH/260001
        return $"DR-{direction}/{year % 100}{number.ToString().PadLeft(4, '0')}";
    }
    ...
    public async Task<RecruitmentRequest> AddRequest(RequestFormDTO data) {
        ...
        // Construire la demande
            ...
            {
                Id = await this.GenerateRequestId(data.Direction, actualYear),
                Post = data.Post,
    ...
    // Récupérer les validations + dernier statut
        ...
        if(request.LastTitularUserId != null) {
            lastTitular = await _userService.GetByIdAsync(request.LastTitularUserId);
        }
    ...
    // Construire le DTO
        return new RequestDetailsDTO
        {
            ...
            LastTitular = lastTitular?.Matricule ?? "N/A",
            Sites = sites.ToArray(),
