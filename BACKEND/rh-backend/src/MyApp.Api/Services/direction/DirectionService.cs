...
namespace MyApp.Api.Services.direction
{
    public interface IDirectionService
    {
        ...
        Task<HashSet<PosteParDirectionDTO>> GetAllPosts();
        Task<HashSet<PosteParDirectionDTO>> GetAllPostsByDirection(string direction);
    }
    ...
    public class DirectionService : IDirectionService
    {
        ...
        public async Task<HashSet<PosteParDirectionDTO>> GetAllPostsByDirection(string direction) {
            try {
                _logger.LogInformation("Recherche des postes dans la direction {dir}", direction);

                return await _repository.GetAllPostsByDirection(direction);
            }
            catch(Exception ex) {
                _logger.LogError(ex, "Erreur lors de la recherche des postes dans la direction {dir}", direction);
                throw;
            }
        }


        public async Task<HashSet<PosteParDirectionDTO>> GetAllPosts() {
            try {
                _logger.LogInformation("Recherche de tous les postes");
                return await _repository.GetAllPosts();
            }
            catch(Exception ex) {
                _logger.LogError(ex, "Erreur lors de la recherche de tous les postes");
                throw;
            }
        }
    }
