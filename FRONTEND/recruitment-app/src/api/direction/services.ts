...
const POSTS_BASE_KEY = ['posts'] as const;
const POSTS_PER_DIR_BASE_KEY = ['postsPerDir'] as const;
...
export const useGetAllPosts = () => {
  const queryKey = [...POSTS_BASE_KEY] as const;

  return useQuery<PosteInfoDTO[], Error>({
    queryKey,
    queryFn: async () => {
      const response = await api.get('/api/Direction/posts');
      return response.data;
    },
  });
};
...
export const useGetAllPostsByDirection = (dirName: string) => {
  const queryKey = [...POSTS_PER_DIR_BASE_KEY, dirName] as const;

  return useQuery<PosteInfoDTO[], Error>({
    queryKey,
    queryFn: async () => {
      const response = await api.get(`/api/Direction/${dirName}/posts`);
      return response.data;
    },
  });
};
