    ...
    public class DirectionRepository : IDirectionRepository
    {
        ...
        public async Task<HashSet<PosteParDirectionDTO>> GetAllPostsByDirection(string direction) {
            string query = @"SELECT 
             ...
             FROM v_postes_par_dir WHERE Direction={0}";

            var posts = await _context.Database
                .SqlQueryRaw<PosteParDirectionDTO>(query, [direction]).AsNoTracking()
                .ToListAsync();

            return posts.ToHashSet();
        }
        ...
        public async Task<HashSet<PosteParDirectionDTO>> GetAllPosts() {
            string query = @"SELECT 
                ...
                departement AS Departement,
                direction AS Direction
             FROM v_postes_par_dir";

            var posts = await _context.Database
                .SqlQueryRaw<PosteParDirectionDTO>(query).AsNoTracking()
                .ToListAsync();
