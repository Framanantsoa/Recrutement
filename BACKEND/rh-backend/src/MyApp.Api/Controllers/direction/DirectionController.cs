...
namespace MyApp.Api.Controllers.direction
{
    ...
    public class DirectionController : ControllerBase
    {
        ...
        [HttpGet("{dir}/posts")]
        [AllowAnonymous]
        public async Task<IActionResult> GetAllPostsByDirection([FromRoute] string dir) {
            // if(!User.Identity?.IsAuthenticated ?? true) {
            //     return Unauthorized(new { data = (object?)null, status = 401, message = "unauthorized" });
            // }

            try {
                var results = await _directionService.GetAllPostsByDirection(dir);
        ...
        public async Task<IActionResult> GetAllPosts() {
            ...
            try {
                var results = await _directionService.GetAllPosts();
                return Ok(new { data = results, status = 200, message = "success" });
            }
            catch (ArgumentException ex) {
                return BadRequest(new { data = (object?)null, status = 400, message = ex.Message });
