...
const RequestTableRow: React.FC<RequestTableRowProps> = ({ request, canEditRequest, canCancelRequest, onEdit, onDelete }) => {
  return (
    <TableRow>
      <TableCell>
        {/* {canViewDetails ?  */}
        <Link style={{ textDecoration: "none" }} to={`/recrutement/demandes/${formatParam(request.id)}/details`}>{request.id}</Link>
        {/* } */}
      </TableCell>
      ...
      <TableCell style={{ textAlign: "center" }}>
        { (canEditRequest && request.status?.toLowerCase()==="en attente") &&
          <EditButton onClick={() => onEdit(request.id)}>
            <Edit size={16} />
          </EditButton>
        }
        
        { (canCancelRequest && request.status?.toLowerCase()==="en attente") && 
